...
库承诺“一定能导出”：当 WASM 路径不可用时自动降级到 SheetJS。
...
| 列宽/冻结/筛选/合并   | ✅               | ❌                                                                                     |
| FormatSpec            | ✅               | ✅（enum/padding/number/date 语义保留，日期输出为可读字符串）                          |
...
兜底是“最后一公里”保障，不是常规路径。出现兜底请优先排查 wasm URL 是否 404、`configureWasm` 是否在导出前调用。
