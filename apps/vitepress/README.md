...
pnpm docs:build    # 构建（先构建 excel-exporter，再拷贝 wasm/worker 到 public/assets）
...
1. 在 `apps/vitepress/package.json` 的 `dependencies` 中加入该包（`workspace:*`）并 `pnpm install`——registry 构建期要读取它的 `package.json` 版本号，live demo 也要 import 它；
...
5. 如需英文版，同步在 `en/packages/<name>/` 写英文页（文件名含数字前缀时须与中文一致）；**暂时只提供中文时不要创建 `en/packages/<name>/` 目录**，`pnpm test` 会跳过该包的镜像校验，en 导航/侧边栏也会自动隐藏仅中文的包；之后补英文时建目录并补齐镜像页面即可；
...
修改 git remote 后首次构建请用 `pnpm docs:build --force` 让 turbo 绕过缓存。
