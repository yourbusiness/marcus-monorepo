...
1. **这是一个 monorepo（大仓库）**：名字叫 `marcus-monorepo`，里面目前只装了一个真正的工具包 `@marcusok/excel-exporter`（一个用 Rust+WASM 加速的高性能 Excel 导出库）。证据见 [pnpm-workspace.yaml](/C:/Users/wangbo/Desktop/marcus-monorepo/pnpm-workspace.yaml)，里面写着 `packages/*`，意思是"packages 文件夹下的每一个子文件夹，都是一个独立的包"。
2. **它是用 pnpm + Turborepo 管理的**：根目录 [package.json](/C:/Users/wangbo/Desktop/marcus-monorepo/package.json) 里 `"packageManager": "pnpm@9.12.0"`，并且所有命令（build/test/lint/typecheck）都是通过 `turbo run xxx` 来跑的（见 [turbo.json](/C:/Users/wangbo/Desktop/marcus-monorepo/turbo.json)）。
3. **它最终会发布到 npm**：包名是 `@marcusok/excel-exporter`，当前版本 `0.1.2`（见 [packages/excel-exporter/package.json](/C:/Users/wangbo/Desktop/marcus-monorepo/packages/excel-exporter/package.json)）。
...
本项目用了 **husky** 这个工具，它在本地会给 git 装上"钩子（hook）"——也就是你每次 `git commit` 的时候，它会自动拦截，先帮你跑一遍检查（见 [.husky/pre-commit](/C:/Users/wangbo/Desktop/marcus-monorepo/.husky/pre-commit) 里写着 `pnpm exec lint-staged`，[.husky/commit-msg](/C:/Users/wangbo/Desktop/marcus-monorepo/.husky/commit-msg) 里写着 `pnpm exec commitlint --edit $1`）。
...
`HUSKY: "0"` 就是告诉 husky："请装作自己不存在"。这是 husky 官方推荐的、在 CI 里禁用钩子的标准做法。证据：[package.json](/C:/Users/wangbo/Desktop/marcus-monorepo/package.json) 的 `"prepare": "husky"` 会在安装时初始化 husky，所以必须用环境变量显式关掉。
...
意思是：这里**故意不写版本号**。这个 action 会自动去读 [package.json](/C:/Users/wangbo/Desktop/marcus-monorepo/package.json) 里的 `"packageManager": "pnpm@9.12.0"`，然后用这个版本。好处是：**你在本地用什么版本的 pnpm，CI 就用什么版本，永远不会错位**。如果哪天升级 pnpm，只要改 package.json 一处，CI 自动跟着变。
...
- [.nvmrc](/C:/Users/wangbo/Desktop/marcus-monorepo/.nvmrc) 文件里写着 `22`。
- [package.json](/C:/Users/wangbo/Desktop/marcus-monorepo/package.json) 里 `"engines": { "node": ">=22.0.0" }`。
- 包 [packages/excel-exporter/package.json](/C:/Users/wangbo/Desktop/marcus-monorepo/packages/excel-exporter/package.json) 里同样 `"engines": { "node": ">=22.0.0" }`。
...
根据 [pnpm-lock.yaml](/C:/Users/wangbo/Desktop/marcus-monorepo/pnpm-lock.yaml)（锁定文件）安装所有依赖。
...
本项目要求所有 commit 都遵守一个固定格式，叫 **Conventional Commits（约定式提交）**。证据在 [.commitlintrc.json](/C:/Users/wangbo/Desktop/marcus-monorepo/.commitlintrc.json)：
...
> 顺带一提：本地你每次 commit 时，[.husky/commit-msg](/C:/Users/wangbo/Desktop/marcus-monorepo/.husky/commit-msg) 也会触发 commitlint。所以同一条规则，本地和 CI 都在守。
...
跑 [package.json](/C:/Users/wangbo/Desktop/marcus-monorepo/package.json) 里的 `"lint": "turbo run lint"`，最终会调到每个包的 lint 命令。本项目用的是 ESLint，配置在 [eslint.config.mjs](/C:/Users/wangbo/Desktop/marcus-monorepo/eslint.config.mjs)。
...
跑 `"typecheck": "turbo run typecheck"`，对应包里是 `tsc --noEmit`（TypeScript 编译器只检查、不输出文件）。它验证"类型对不对"——比如你把一个数字传给了一个要字符串的函数，它就会报错。这是 TypeScript 项目最关键的防线之一。类型规则在 [tsconfig.base.json](/C:/Users/wangbo/Desktop/marcus-monorepo/tsconfig.base.json) 里，开了 `strict: true`（最严格）。
...
跑 `"test": "turbo run test"`，最终用 **vitest** 跑所有测试（测试在 [packages/excel-exporter/src/**tests**](/C:/Users/wangbo/Desktop/marcus-monorepo/packages/excel-exporter/src/__tests__) 下，比如 builder、format、stream、fallback、routing 等）。

重点是这个环境变量 **`RUN_PERF: "0"`**，它的来龙去脉很有意思，是项目踩过坑后加的：

项目里有一个 [performance.test.ts](/C:/Users/wangbo/Desktop/marcus-monorepo/packages/excel-exporter/src/__tests__/performance.test.ts)，它是**性能测试**——比如测"导出 10 万行 Excel 要多久，有没有超过 2 秒"。代码里是这样写的：
...
这背后还引出一个 `PERF_TIGHT` 的设计（同一文件里 `SLACK` 那行）：本地想跑严格模式可以设 `PERF_TIGHT=1`（把宽松倍数从 1.5 降到 1.0）。而 [turbo.json](/C:/Users/wangbo/Desktop/marcus-monorepo/turbo.json) 的 `globalEnv` 里专门列了 `RUN_PERF` 和 `PERF_TIGHT`：
...
跑 `"build": "turbo run build"`，对应包里是 `tsup`（一个打包工具，配置在 [packages/excel-exporter/tsup.config.ts](/C:/Users/wangbo/Desktop/marcus-monorepo/packages/excel-exporter/tsup.config.ts)）。它把 TypeScript 源码编译、打包成最终能被别人 `import` 用的 `dist/` 产物。

这一步放在最后，因为构建最耗时，前面（lint/typecheck/test）如果有问题，早点失败、早点停下，省时间。

> Turborepo 的智能之处（见 [turbo.json](/C:/Users/wangbo/Desktop/marcus-monorepo/turbo.json)）：`build` 任务声明了 `"dependsOn": ["^build"]`，意思是"先把我依赖的包构建好，再构建我"；还声明了 `"outputs": ["dist/**"]`，让 Turborepo 知道构建产物在哪，可以缓存。不过当前只有一个包，暂时看不出多包编排的效果。
...
| 2    | 装 pnpm（自动读版本） | 工具版本一致     | [package.json](/C:/Users/wangbo/Desktop/marcus-monorepo/package.json) 的 `packageManager`                               |
| 3    | 装 Node 22 + 缓存     | 运行环境一致     | [.nvmrc](/C:/Users/wangbo/Desktop/marcus-monorepo/.nvmrc)、`engines`                                                    |
| 4    | install（冻结锁文件） | 依赖版本一致     | [pnpm-lock.yaml](/C:/Users/wangbo/Desktop/marcus-monorepo/pnpm-lock.yaml)                                               |
| 5    | commitlint（仅 PR）   | 提交信息规范     | [.commitlintrc.json](/C:/Users/wangbo/Desktop/marcus-monorepo/.commitlintrc.json)                                       |
| 6    | lint                  | 代码风格规范     | [eslint.config.mjs](/C:/Users/wangbo/Desktop/marcus-monorepo/eslint.config.mjs)                                         |
| 7    | typecheck             | 类型正确         | [tsconfig.base.json](/C:/Users/wangbo/Desktop/marcus-monorepo/tsconfig.base.json)                                       |
| 8    | test（跳过性能测试）  | 功能正确         | [packages/excel-exporter/src/**tests**](/C:/Users/wangbo/Desktop/marcus-monorepo/packages/excel-exporter/src/__tests__) |
| 9    | build                 | 能成功打包出产物 | [tsup.config.ts](/C:/Users/wangbo/Desktop/marcus-monorepo/packages/excel-exporter/tsup.config.ts)                       |
