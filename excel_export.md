...
export function applyFormat(value: unknown, spec: FormatSpec): string | number {
  switch (spec.type) {
    case 'enum': return spec.map[String(value)] ?? spec.fallback ?? String(value);
    case 'date': { /* 转 yyyy-MM-dd，省略实现细节 */ return String(value); }
    case 'datetime': { return String(value); }
    case 'number': { const n = Number(value); return spec.thousands ? n.toLocaleString('en-US', {minimumFractionDigits: spec.decimals ?? 0}) : n.toFixed(spec.decimals ?? 0); }
    case 'padding': { const s = String(value); return spec.align === 'left' ? s.padEnd(spec.length, spec.fill) : s.padStart(spec.length, spec.fill); }
  }
}
...
export async function exportAsStream(sheets: SheetConfig[]): Promise<StreamResult> {
  ...
  for (const config of sheets) {
    ...
    // 表头行
    writer.writeRow(
      config.columns.map((c) => ({ value: c.header, cellType: 'sharedString' }))
    );
    // 数据行（逐行写入，内存占用恒定）
    for (const item of config.data) {
      const cells: StreamingCellInput[] = config.columns.map((col) => {
        ...
        return { value: raw == null ? '' : String(raw), cellType: 'sharedString' };
      });
...
export async function exportInWorker(
  ...
  try {
        ...
        sheets: options.sheets.map((s) => ({
          ...
          columns: s.columns.map((c) => {
            if (c.format && typeof c.format === 'function') {
              console.warn('[excel-exporter] column "' + c.key + '" uses a function format, which is stripped for worker mode. Use FormatSpec for worker compatibility.');
              const { format, ...rest } = c;
              return rest;
            }
            return c;
    ...
    return {
      success: true, blob, engine: 'modern-xlsx', mode: mode === 'stream' ? 'stream' : 'worker',
      duration: performance.now() - start, rowCount: totalRows,
    };
  } catch (e) {
...
function pickMode(options: ExportOptions, totalRows: number): PickedMode {
  ...
  if (explicit === 'worker') return { mode: 'worker', workerMode: totalRows >= 50_000 ? 'stream' : 'workbook' };
  ...
  if (!isBrowser) {
    return totalRows >= 50_000 ? { mode: 'stream', workerMode: 'stream' } : { mode: 'main' };
  }
  ...
  if (picked.mode === 'main' || (picked.mode === 'stream' && typeof window === 'undefined')) {
    try {
      ...
      if (picked.workerMode === 'stream') {
        ...
        result = { success: true, blob, engine: 'modern-xlsx', mode: 'stream', duration: performance.now() - start, rowCount };
      } else {
        const builder = await WorkbookBuilder.create();
        options.sheets.forEach((s) => builder.addSheet(s));
        const bytes = await builder.toBuffer();
        const blob = new Blob([bytes], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
        result = { success: true, blob, engine: 'modern-xlsx', mode: 'main', duration: performance.now() - start, rowCount: totalRows };
      }
...
export async function exportWithSheetJS(
  ...
  try {
    ...
    for (const s of options.sheets) {
      const aoa = [s.columns.map((c) => c.header), ...s.data.map((row) => s.columns.map((c) => row[c.key] ?? ''))];
    ...
    const totalRows = options.sheets.reduce((s, sh) => s + sh.data.length, 0);
    return {
      success: true, blob, engine: 'sheetjs', mode: 'main',
      duration: performance.now() - start, rowCount: totalRows,
...
if ('requestIdleCallback' in window) {
  (window as any).requestIdleCallback(() => {
    getWasmLoader().ensureLoaded().catch(() => {/* 预热失败静默，正式导出时会重试 */});
  }, { timeout: 4000 });
}
```
  ...
  plugins: [
    {
      ...
      buildStart() {
        ...
        copyFileSync(`${mxlsxDist}/modern-xlsx.wasm`, 'public/assets/modern-xlsx.wasm');
        // 2. 本库的 Worker 脚本
        ...
        if (!statSync(workerSrc, { throwIfNoEntry: false })) {
          throw new Error(`[excel-exporter] 未找到 export.worker.mjs：请先在库包内 pnpm build（产物在 dist/export.worker.mjs）。目录：${exporterDist}`);
        }
...
function genRows(n: number) {
  return Array.from({ length: n }, (_, i) => ({
    id: i, name: `user_${i}`, amount: Math.random() * 10000, createdAt: new Date(),
  }));
}
...
describe('performance', () => {
  ...
  beforeAll(async () => {
    ...
    const require = createRequire(import.meta.url);
    const wasmPath = dirname(require.resolve('modern-xlsx')) + '/modern-xlsx.wasm';
    ...
    await exportExcel({
      filename: 'warmup', download: false, mode: 'main',
  ...
  it('10k rows × 4 cols WASM-core < 200ms (4列基准, 回归下限)', async () => {
    ...
    const r = await exportExcel({
      filename: 'perf-10k', download: false, mode: 'main',
      sheets: [{ name: 's', columns: [
        { key: 'id', header: 'ID' }, { key: 'name', header: 'Name' },
        { key: 'amount', header: 'Amt' }, { key: 'createdAt', header: 'Date' },
      ], data: genRows(10_000) }],
    });
  ...
  it('50k rows × 4 cols WASM-core < 700ms (4列基准, 回归下限)', async () => {
    ...
    const t0 = performance.now();
    const r = await exportExcel({
      filename: 'perf-50k', download: false, mode: 'main',
      sheets: [{ name: 's', columns: [
        { key: 'id', header: 'ID' }, { key: 'name', header: 'Name' },
        { key: 'amount', header: 'Amt' }, { key: 'createdAt', header: 'Date' },
      ], data: genRows(50_000) }],
  ...
  it('100k rows × 4 cols WASM-core < 1000ms (4列基准, stream, 回归下限)', async () => {
    ...
    const t0 = performance.now();
    const r = await exportExcel({
      filename: 'perf-100k', download: false, mode: 'stream',
      sheets: [{ name: 's', columns: [
        { key: 'id', header: 'ID' }, { key: 'name', header: 'Name' },
        { key: 'amount', header: 'Amt' }, { key: 'createdAt', header: 'Date' },
  ...
  it('format function overhead should not dominate', async () => {
    ...
    const lookup = { ...noop, format: (v: unknown) => ({ 0: 'A', 1: 'B' } as Record<number, string>)[v as number] ?? String(v) };
    const data = Array.from({ length: 10_000 }, (_, i) => ({ id: i }));
    const makeSheet = (col: typeof noop) => ({ name: 's', columns: [col], data });

    const t0 = performance.now();
    await exportExcel({ filename: 'fmt-noop', download: false, mode: 'main', sheets: [makeSheet(noop)] });
    const tNoop = performance.now() - t0;

    const t1 = performance.now();
    await exportExcel({ filename: 'fmt-id', download: false, mode: 'main', sheets: [makeSheet(identity)] });
    const tId = performance.now() - t1;

    const t2 = performance.now();
    await exportExcel({ filename: 'fmt-lookup', download: false, mode: 'main', sheets: [makeSheet(lookup)] });
...
test('worker-mode 5万行 × 4列: 端到端 < 700ms 且无 longtask (4列基准)', async ({ page }) => {
...
test('worker-mode 10万行 × 4列: 端到端 < 2000ms 且无 longtask (4列基准, 最紧张验收点)', async ({ page }) => {
  // 4 列基准（见 1.2 列数缩放规则）。列数增加时按 budget(4列)×(C/4) 放宽阈值。
